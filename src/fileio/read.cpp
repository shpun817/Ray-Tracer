	...
	const mmap& materials, TransformNode *transform )
{
	if( name == "translate" ) {
		...
		if( name == "sphere" ) {
			...
			bool cap = true;
			maybeExtractField(child, "capped", cap);
			obj = new Cylinder( scene, mat, cap);
		} else if( name == "cone" ) {
			double height = 1.0;
...
static void processObject( Obj *obj, Scene *scene, mmap& materials )
{
	...
	if( name == "directional_light" ) {
		...
		scene->add(new SpotLight(scene,
			tupleToVec(getField(child, "position")), //pos
			tupleToVec(getField(child, "direction")).normalize(), //ori
			tupleToVec(getColorField(child)), //color
			getField(child, "angle")->getScalar() //ang
		...
		scene->addAmbientLight( tupleToVec(getColorField(child)) );
	}
	else if( 	name == "sphere" ||
				name == "box" ||
				name == "cylinder" ||
