...
public:
    ...
    ray jiggle(double deviation) {
        return ray(p, (d + vec3f(   frand(deviation * 2) - deviation,
                                    frand(deviation * 2) - deviation,
                                    frand(deviation * 2) - deviation)).normalize());
    }
protected:
	vec3f p;
	vec3f d;
